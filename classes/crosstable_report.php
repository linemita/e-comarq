...
class CrossTableReport
{
	...
	public function isProjectDB()
	{
		...
		$isDB = false;
		if("calificacion" == $this->xml_array['tables'][0])
			$isDB = true;
		if("categorias" == $this->xml_array['tables'][0])
			$isDB = true;
		if("comentarios" == $this->xml_array['tables'][0])
			$isDB = true;
		if("departamento" == $this->xml_array['tables'][0])
			$isDB = true;
		if("municipio" == $this->xml_array['tables'][0])
			$isDB = true;
		if("productos" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tipopago" == $this->xml_array['tables'][0])
			$isDB = true;
		if("usuario" == $this->xml_array['tables'][0])
			$isDB = true;
		if("productos" == $this->xml_array['tables'][0])
			$isDB = true;
		return $isDB;
