...
$page_layouts["municipio_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("Departamento_id") . "=" . make_db_value("Departamento_id",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) 
{
	...
	//	Departamento_id - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("Departamento_id", $data, $keylink);
			$row["Departamento_id_value"] = $value;
			$format = $pSet->getViewFormat("Departamento_id");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("Departamento_id")))
				$class = ' rnr-field-number';
			$row["Departamento_id_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("municipio", "detailspreview"));
