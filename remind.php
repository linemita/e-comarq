...
$layout = new TLayout("remind2", "BreezeBlue_white", "MobileBlue_white");
$layout->version = 2;
...
if (@$_POST["btnSubmit"] == "Remind")
{
	...
	{	
		...
		if($tosearch)
		{
			...
			$strSQL = $selectClause." from ".$pageObject->connection->addTableWrappers("usuario")." where ".$sWhere;
...
$layout = new TLayout("remind_success2", "BreezeBlue_white", "MobileBlue_white");
$layout->version = 2;
$layout->blocks["top"] = array();
...
$layout->skinsparams["3"] = array("button"=>"button1");
				...
				$message.= "Recordar contraseña"."\r\n";
				$message.= "Has solicitado recordar tu usuario y contraseña"." ".$url2."\r\n";
				$message.= "Nombre de usuario".": ".$data[$cUserNameField]."\r\n";
				$message.= "Contraseña".": ".$password."\r\n";
				$sentMailResults = runner_mail(array('to' => $data[$cEmailField], 'subject' => "Recordar contraseña", 'body' => $message));
		...
		if(!$reminded)
		{
			if($sentMailResults['message'])
				...
				$strMessage = "Usuario"." ".$strUsernameEmail." "."no registrado";
		}
